...
// 	if strings.Count(q._Stmt, clause) > 0 {
// 		q._Stmt += " and"
// 	} else {
// 		q._Stmt += " " + clause
// 	}
...
// 	} else {
// 		q.error = fmt.Errorf("expression is not allowed")
// 	}
// }

// func (q *QueryBuilder) between(column string, logic string, fromValue interface{}, toValue interface{}) {
// 	if strings.Count(q._Stmt, q.currentTag) > 0 {
// 		q._Stmt += " " + logic
// 	} else {
// 		q._Stmt += " " + q.currentTag
// 	}
// 	q._Stmt += fmt.Sprintf(" %s $%d and $%d", column, len(q.args)+1, len(q.args)+2)
// 	q.args = append(q.args, fromValue, toValue)
// }

// func (q *QueryBuilder) nullableClause(column string, logic string) {
// 	if strings.Count(q._Stmt, q.currentTag) > 0 {
// 		q._Stmt += fmt.Sprintf(" %s %s", logic, column)
// 	} else {
// 		q._Stmt += fmt.Sprintf(" %s %s", q.currentTag, column)
// 	}
// }

// func (q *QueryBuilder) rangeClause(column string, logic string, values ...any) {
// 	if strings.Count(q._Stmt, q.currentTag) > 0 {
// 		q._Stmt += fmt.Sprintf(" %s %s (", logic, column)
// 	} else {
// 		q._Stmt += fmt.Sprintf(" %s %s (", q.currentTag, column)
...
// 		q._Stmt += fmt.Sprintf("$%d", len(q.args)+1)
// 		if k < len(values)-1 {
// 			q._Stmt += ","
// 		}
...
// func (q *QueryBuilder) expressionClause(column string, logic string, value interface{}) {
// 	if strings.Count(q._Stmt, q.currentTag) > 0 {
// 		q._Stmt += " " + logic
// 	} else {
// 		q._Stmt += " " + q.currentTag
// 	}
// 	q._Stmt += fmt.Sprintf(" %s $%d", column, len(q.args)+1)
